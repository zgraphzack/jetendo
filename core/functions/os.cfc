...
<cffunction name="zUpdateGlobalMVCData" localmode="modern" access="public" output="no">
	...
	if(directoryexists(request.zos.installPath&"core/mvc/")){
		if(fileexists("#request.zos.installPath#core/mvc-cache.cfc")){
		...
		if(output NEQ ""){
			...
			if(qD.recordcount GT 1){
				...
				if(request.zos.isExecuteEnabled){
					if(hash(tempComponentString) NEQ hash(application.zcore.functions.zreadfile("#request.zos.installPath#core/mvc-cache.cfc"))){
						application.zcore.functions.zwritefile(arguments.ss.serverglobals.serverhomedir&"mvc-cache.cfc", tempComponentString);
						application.zcore.functions.zClearCFMLTemplateCache();
	...
	}else if(request.zos.isExecuteEnabled){
		mvcCacheCom=createobject("component","zcorerootmapping.mvc-cache");
...
<cffunction name="zPublishMenu" localmode="modern" output="no" returntype="any">
...
<cfsavecontent variable="db.sql">
	SELECT *, menu.site_id FROM #db.table("menu", request.zos.zcoreDatasource)# menu 
	WHERE menu.menu_id = #db.param(arguments.menu_id)# AND 
	menu.site_id = #db.param(arguments.site_id)#
    
</cfsavecontent><cfscript>qView=db.execute("qView");</cfscript> 
...
##zMenuDiv#qView.menu_id# a {
	...
	padding: #qView.menu_padding_height#px <cfif qView.menu_enable_responsive EQ 1>0px<cfelse>#qView.menu_padding_width#px</cfif> #qView.menu_padding_height#px <cfif qView.menu_enable_responsive EQ 1>0px<cfelse>#qView.menu_padding_width#px</cfif>;
...
##zMenuDiv#qView.menu_id# a:active, ##zMenuDiv#qView.menu_id# li.zMenuHvr a {
	...
	<cfif qView.menu_background_over_image NEQ ""> background-image:url(/zupload/menu/#qView.menu_id#/#qView.menu_background_over_image#); background-position:top; </cfif>
...
<!--- this is the popup links rollover --->
##zMenuDiv#qView.menu_id# ul a:hover {
	<cfif qView.menu_popup_background_over_image NEQ ""> background-image:url(/zupload/menu/#qView.menu_id#/#qView.menu_popup_background_over_image#) !important; background-position:top; <cfelse>
    background:none;
...
##zMenuDiv#qView.menu_id#{ float:left;<cfif qView.menu_font_color NEQ "">color:###qView.menu_font_color#;<cfelse>color:##FFFFFF;</cfif>background-color:###qView.menu_background_color#;<cfif qView.menu_background_image NEQ "">background-image:url(/zupload/menu/#qview.menu_id#/#qview.menu_background_image#);background-position:top; </cfif><cfif qView.menu_size_limit NEQ 0>width:#qView.menu_size_limit#px;</cfif> } 
##zMenuDiv#qView.menu_id# li li {font-size:#qView.menu_popup_font_size#px;} 
##zMenuDiv#qView.menu_id# li {font-size:#qView.menu_button_font_size#px;}<cfif qView.menu_vertical EQ '1'>
<!---  .zMenuBarDiv li{width:100%;clear:both;} --->
