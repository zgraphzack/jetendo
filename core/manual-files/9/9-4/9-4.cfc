...
<li><a href="##Lucee">CFML Server Security</a></li>
...
<p>AppArmor is configured to run in enforce mode for MariaDB, PHP5-FPM, Lucee and Nginx.</p>
...
<p>Because Lucee/Tomcat runs as the group (Nginx), it is necessary for have UMASK 0007 for Lucee so that the group has write access for new files created by Jetendo. we set this UMASK during Lucee/Tomcat start-up.</p>
...
<p>On both production and development, there are extra layers of security besides the filesystem security including Lucee's sandbox and AppArmor which have both been configured to be specific to Jetendo's needs.</p>
...
<p>Nginx acts as a reverse proxy for Tomcat/Lucee.  Nginx serves all the static files, so that Tomcat only has to serve dynamic requests.   Nginx acts like a firewall for Lucee because it prevents direct access to the Lucee administrator via public URLs.</p>
...
<p>Implement Nginx proxy caching to cache more of the dynamic requests to improve overall performance and resistance to denial of service attacks.  Nginx can serve 1,000 more static requests per second then a complex dynamic request to PHP or Lucee.</p>

<h2 id="Lucee">Lucee &amp; Tomcat Security</h2>
<p>Lucee is a free open source alternative to Adobe Coldfusion.  One of the best features of Lucee is that doesn't cause you to pay anything for its core language and security features.  Adobe Coldfusion Enterprise costs $8,499 (Source Adobe.com on 1/21/2014) to provide the extra security isolation provided via multiple installations or multiple Java server contexts.  However, with Lucee, you can install as many copies as you wish, or you can use the sandboxing and contexts features to do this.  If you need many servers later, you don't end up paying more. It is also easier to distribute Lucee with commercial applications thanks to it's generous LGPL license.</p>
<h3>What is the Lucee sandbox used for? </h3>
<p>The sandbox  access settings in Lucee's server administrator let you define the level of access specific paths on the system have. Internally, it relies on Java &amp; tomcat security model, which is quite robust. Jetendo has been engineered to allow an extremely restricted security integration with Lucee.  Lucee has a number of advanced features that may be useful for development and specific users. However, with security, you may want to disable or limit these features.</p>
<p>Jetendo is fully functional when the following features are disabled in Lucee's security sandbox:</p>
<ul>
...
<p>Additionally, the paths Jetendo needs to allow Lucee to access are isolated in a way that allows parts of Jetendo to be invisible to Lucee.</p>
<p>With no ability to call Java code directly and no ability to execute other processes on the system, Lucee is only able to execute pure CFML. This makes Jetendo excellent in a shared hosting environment. We plan to offer Jetendo exclusive shared hosting in the future. Multiple installations of Jetendo could run on separate Java contexts safely, without being able to attack each other.</p>
<h3>Custom AppArmor Profile for Lucee + Jetendo</h3>
<p>Lucee has a custom AppArmor profile configured to be specific to Jetendo's requirements. Because Lucee is only running CFML, the AppArmor profile is extremely simple to maintain, due to only a few dozen rules being needed. </p>
<p>After adding AppArmor, Jetendo has at least 4 layers of security protecting access to files. The filesystem permissions, AppArmor profile, the Lucee Sandbox and validation in Jetendo's API. An attacker may need to find a flaw in more then one of these systems to gain additional access to the rest of the system.</p>
<h2 id="async">Asynchronous message passing in Jetendo</h2>
<p>With no direct system access, you may think that Jetendo must be more limited in what it can do. However, this is not true. You don't need to directly access the database data files in order to use a database, and likewise you shouldn't need to access a unconfined process directly.  To work around this, Jetendo has implemented all of its advanced shell integration features through message passing between Lucee and PHP cron jobs that are constantly running.</p>
<p>This process of passing messages back and forth occurs through the database or filesystem in a location where both Lucee and the PHP cron jobs can read and write.  Because messages are processed sequentially one at a time, it is not possible for a task to consume all system resources and slow down the system. If many users attempt to resize images at the same time, it may take slightly longer for them to all complete, but this would only be a few seconds, not very noticeable. If a task takes longer then a minute, another PHP cron job will be fired off, and then there will be some parallel activity. </p>
...
<p>We try to minimize the amount of PHP we introduce into the Jetendo project because it is primarily intended to be a pure CFML application. PHP is much easier to use for now when dealing with shell integration and image resizing.  We will evaluate Lucee's CLI again when it improves.</p>
<h3>PHP + Nginx</h3>
...
<p>Session tracking is used to keep track of what pages a user visited after submitting an inquiry form.   It is also used to pass messages between pages without using the database.  Session memory in Lucee is very fast, but you want to protect against using too much memory so that Lucee is stable.  Someone attacking the system in order to overwhelm it's capacity is often called a Denial of Service attack.   Jetendo attempts to prevent denial of service attacks by limiting connections and preventing excessive memory usage.</p>
