...
application.zcore.functions.zRedirect("/z/server-manager/admin/site-import/index?zsid=#request.zsid#");
	...
	application.zcore.functions.zSecureCommand("untarZipSitePath"&chr(9)&fileName&chr(9)&curDate, 3600);
	...
	for(row in qDir){
		if(row.directory NEQ "#curImportPath#temp/database" and row.name NEQ "." and row.name NEQ ".."){
			...
			if(debug) writeoutput(db.sql&" | site_id is #globals.site_id#<br />");
			db.execute("qDelete");
		}
	}
	for(n in dsStruct){
