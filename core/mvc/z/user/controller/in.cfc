...
<cffunction name="confirmed" localmode="modern" access="remote" output="yes">
	<cfscript>
	var zpagenav='<a href="#request.zos.globals.domain#/">Home</a> /';
	if(structkeyexists(form, 'e') EQ false or structkeyexists(form, 'k') EQ false){
		application.zcore.functions.zRedirect(request.zos.globals.domain&'/z/user/preference/index');
	}
	...
	<a href="#request.zos.globals.domain#/z/user/preference/index?e=#urlencodedformat(form.e)#&k=#urlencodedformat(form.k)#&action=form">#request.zos.globals.domain#/z/user/preference/index</a></p>
	<p><a href="#request.zos.globals.domain#/z/user/privacy/index">View our privacy policy</a></p>
...
<cffunction name="index" localmode="modern" access="remote" output="yes">
	...
	if(structkeyexists(form, 'e') and structkeyexists(form, 'k')){
		...
		ts.key=form.k;
		writedump(form.k);
		ts.redirectURL=request.zos.globals.domain&"/z/user/in/confirmed?e=#urlencodedformat(form.e)#&k=#urlencodedformat(form.k)#";
