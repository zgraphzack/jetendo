...
<cffunction name="download" localmode="modern" access="remote" roles="member">
	...
	echo('"Email","Company","First Name","Last Name","Phone","Opt In","Opt In Confirmed"'&chr(10));
	...
	qU=db.execute("qU");
	loop query="qU"{
		echo('"'&qU.user_username&'","'&replace(qU.member_company,'"', "'", 'all')&'","'&qU.user_first_name&'","'&qU.user_last_name&'","'&qU.user_phone&'","'&qU.user_pref_email&'","'&qU.user_confirm&'"'&chr(10));
	}
