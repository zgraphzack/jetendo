...
<cffunction name="index" localmode="modern" access="remote" roles="member">
	...
	<!--- 
	interface idea:
		select search criteria group to display
			set children record, select search criteria to display - allow more then one
				allow selecting layout
				select  sort order (by count desc or alphabetic)
		need select dropdown in site options
		need "plug-in" friendly site options
		
		need to finish search2.cfc
		
		allow each item to have customizable title/ meta title, keywords, description and url
	
	box
		finish search2.cfc - apply to search-form and search.cfc
			column of city, subdivision, price range or property type, frontage, view, etc
				get group counts
			make easily skinnable
				use html compiler finally? hardcode for now? what?
			integrate with site options?
				allow plug-in system for site option data types, convert existing ones to the plug-in format.  
					Use javascript / ajax for the interface so they don't all get loaded
					allow select menu
					allow 
	...
	<!--- 
		...
		manual_listing_unique_id
		manual_listing_id   0-#manual_listing_unique_id# and site_id =
		manual_listing_address
		site_id=
		...
		build the mls-provider/manual.cfc
		
		
		listing_id                 varchar(15)    utf8_general_ci  NO      PRI     (NULL)           select,insert,update,references           
		listing_data_remarks       varchar(2000)  utf8_general_ci  NO      MUL     (NULL)           select,insert,update,references           
		listing_data_address       varchar(100)   utf8_general_ci  NO              (NULL)           select,insert,update,references           
		listing_data_zip           varchar(5)     utf8_general_ci  NO              (NULL)           select,insert,update,references           
		listing_data_detailcache1  text           utf8_general_ci  NO              (NULL)           select,insert,update,references           
		listing_data_detailcache2  text           utf8_general_ci  NO              (NULL)           select,insert,update,references           
		listing_data_detailcache3 
		 --->
	...
	<cfif searchtext NEQ ''>
		...
		if(manual_listing.manual_listing_id = #db.param(searchtext)#, #db.param(1)#,#db.param(0)#) matchingId, 
		#db.trustedSQL("if(concat(manual_listing.manual_listing_id,' ', cast(manual_listing.manual_listing_price as char(11)),' ',manual_listing.manual_listing_address,' ',manual_listing.manual_listing_mls_number)")# like #db.param('%#searchTextOriginal#%')#,#db.param(1)#,#db.param(0)#) as matchPriceAddress, 
	</cfif>
	...
	<cfif searchtext NEQ ''>
		<cfif searchexactonly EQ 1>
			and (#db.trustedSQL("concat(manual_listing.manual_listing_id,' ',cast(manual_listing.manual_listing_price as char(11)),' ',manual_listing.manual_listing_address,' ',manual_listing.manual_listing_mls_number")#) like #db.param('%#searchTextOriginal#%')# or 
			manual_listing.manual_listing_remarks like #db.param('%#searchTextOriginal#%')# )
		<cfelse>
			and 
			
			(#db.trustedSQL("concat(manual_listing.manual_listing_id,' ',cast(manual_listing.manual_listing_price as char(11)),' ',manual_listing.manual_listing_address,' ',manual_listing.manual_listing_mls_number)")# like #db.param('%#searchTextOriginal#%')# or 
			(
			((
			<cfif application.zcore.enableFullTextIndex>
	...
	GROUP BY manual_listing.manual_listing_id 
	ORDER BY
	<cfif searchtext NEQ ''>
	...
	<cfif qSite.recordcount NEQ 0>
		<table style="border-spacing:0px; width:100%;" class="table-list">
			...
			<tr <cfif qsite.currentRow MOD 2 EQ 0>class="row1"<cfelse>class="row2"</cfif>>
				<td style="vertical-align:top; width:30px; ">#qSite.manual_listing_id#</td>
				<td style="vertical-align:top; width:100px; "><cfif manual_listingphoto99 NEQ "">
						<img alt="Image" src="#request.zos.globals.domain&manual_listingphoto99#" width="100" height="70" /></a>
						<cfelse>
						&nbsp;
					</cfif></td>
				...
				<td style="vertical-align:top; "><a href="##viewlink" target="_blank">View</a> | <a href="/z/listing/admin/manual-listing/edit?manual_listing_mls_id=#qSite.manual_listing_mls_id#&manual_listing_id=#qSite.manual_listing_id#&return=1">Edit</a>
					| <a href="/z/listing/admin/manual-listing/delete?manual_listing_id=#qSite.manual_listing_id#&return=1">Delete</a></td>
			</tr>
			</cfloop>
		</table>
	</cfif>
...
qCheck=db.execute("qCheck");
	...
	<cfif structkeyexists(form, 'confirm')>
		...
		form.site_id=request.zos.globals.id;
			application.zcore.functions.zdeleterecord("manual_listing","site_id,manual_listing_id",request.zos.zcoreDatasource);
            application.zcore.status.setStatus(request.zsid, 'Page deleted.');
            application.zcore.functions.zRedirect('/z/listing/admin/manual-listing/index?zsid=#request.zsid#');
            </cfscript>
		<cfelse>
		<h2> Are you sure you want to delete this manual_listing?<br />
			<br />
...
<cffunction name="update" localmode="modern" access="remote" roles="member">
    ...
    application.zcore.adminSecurityFilter.requireFeatureAccess("Manage Listings", true);
        ...
        if(structkeyexists(form, 'manual_listing_created_datetime') and form.manual_listing_created_datetime NEQ false){
            if(isdate(form.manual_listing_created_datetime) EQ false or (form.manual_listing_created_datetime NEQ '' and isdate(form.manual_listing_created_datetime) EQ false)){		
                ...
                if(form.method EQ "update"){
                    application.zcore.functions.zRedirect("/z/listing/admin/manual-listing/edit?manual_listing_mls_id=#form.manual_listing_mls_id#&manual_listing_id=#form.manual_listing_id#&zsid="&request.zsid);
                }else{
        ...
        if(form.method EQ 'insert'){
			form.manual_listing_id=randrange(1000000,9999999);
			...
			form.manual_listing_id="0-"&form.manual_listing_unique_id;
            ...
            if(application.zcore.functions.zUpdate(ts) EQ false){
                application.zcore.status.setStatus(request.zsid, 'Page with this name already exists.  Please type a unique name',form,true);
                application.zcore.functions.zRedirect('/z/listing/admin/manual-listing/edit?manual_listing_mls_id=#form.manual_listing_mls_id#&zsid=#request.zsid#&manual_listing_id=#form.manual_listing_id#');
            }
        }
...
<cffunction name="edit" localmode="modern" access="remote" roles="member">
	...
	<h2>
		...
		listing</h2>
	<cfscript>
        form.manual_listing_id=application.zcore.functions.zso(form, 'manual_listing_id');
        ...
        application.zcore.functions.zQueryToStruct(qmanual_listing, form,'manual_listing_id,site_id,');
        application.zcore.functions.zStatusHandler(request.zsid,true, false, form);
	...
	<table style="width:100%; border-spacing:0px;" class="table-list">
		...
		<tr>
			...
			<td style="vertical-align:top; "><input type="text" name="manual_listing_metatitle" value="#HTMLEditFormat(form.manual_listing_metatitle)#" maxlength="150" size="100" />
				<br />
				(Meta title is optional and overrides the &lt;TITLE&gt; HTML element to be different from the visible page title.) </td>
		</tr>
		<tr>
