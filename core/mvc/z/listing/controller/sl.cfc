...
if(isDefined('session.zos.user.id')){
	if(isDefined('session.zos.listing.savedListingUserLoaded') EQ false){
		...
		if(qC.recordcount NEQ 0){
			...
			if(isDefined('session.zos.listing.savedListingStruct') EQ false){
				session.zos.listing.savedListingStruct=structnew();
			}
			for(i=1;i LTE arraylen(arrKey);i++){
				session.zos.listing.savedListingStruct[arrKey[i]]=true;
			}
			if(isDefined('session.zos.listing.savedCountStruct') EQ false){
				session.zos.listing.savedContentStruct=structnew();
			}
			arrKey=listtoarray(qc.saved_content_idlist);
			for(i=1;i LTE arraylen(arrKey);i++){
				session.zos.listing.savedContentStruct[arrKey[i]]=true;
			}
			
		}else if(isDefined('session.zos.listing.savedContentStruct') and isDefined('session.zos.listing.savedListingStruct') and (structcount(session.zos.listing.savedContentStruct) NEQ 0 or structcount(session.zos.listing.savedListingStruct) NEQ 0)){
			application.zcore.listingCom.updateUserSavedListings();	
			//application.zcore.functions.zdump(request.zos.arrQueryLog);
			//application.zcore.functions.zabort();
		}
		session.zos.listing.savedListingUserLoaded=true;
	}
}else if(isDefined('session.zos.listing.savedListingUserLoaded')){
	ts=structnew();
	ts.name="savedListingStruct";
	ts.value="";
	ts.expires="never";
...
if(isDefined('session.zos.listing.savedContentStruct') eq false){
	session.zos.listing.savedContentStruct=structnew();
...
application.zcore.functions.zHeader("Content-type","text/javascript");
    ...
    <td style="text-align:right;" class="zSavedList-left-td">Actions: <a href="##" onclick="zShowModalStandard('/z/listing/sl/index?saveAct=inquiry', 540, 630);return false;" rel="nofollow" class="zSavedList-link">Send all to us</a> | <cfif isDefined('session.zos.user.id') EQ false><a href="/z/user/preference/index?action=form&zSignupMessage=#URLEncodedFormat("By creating an account below, your saved listings will be stored with the email and password you provide making it easy to retrieve them later.")#" rel="nofollow">Save to New Account</a> | </cfif><a href="/z/listing/sl/index?saveAct=removeall&returnURL=#urlencodedformat(request.zos.cgi.http_referer)#" rel="nofollow">Remove All</a></td>
    </tr>
    <tr><td colspan="2">
...
<style type="text/css">body{background:none !important;} body, table{ background-color:##FFF !important; color:##000 !important;} a:link, a:visited{ color:##369 !important; } </style><meta name="ROBOTS" content="NOINDEX,NOFOLLOW" />');
	...
	ts.arrMLSPid=structkeyarray(session.zos.listing.savedListingStruct);
	//ts.debug=true;
	ts.perpage=200;
	...
	contentIdList=structkeylist(session.zos.listing.savedContentStruct,"','");
...
<td style="text-align:center; border-right:1px solid ##999;"><a href="<cfif qContent.content_url_only NEQ ''>#application.zcore.functions.zForceAbsoluteUrl(request.zos.globals.domain,qContent.content_url_only)#<cfelse>#request.zos.globals.domain#<cfif qContent.content_unique_name NEQ ''>#qContent.content_unique_name#<cfelse>/#application.zcore.functions.zURLEncode(qContent.content_name,'-')#-#application.zcore.app.getAppData("content").optionstruct.content_config_url_article_id#-#qContent.content_id#.html</cfif></cfif>" style=" font-weight:normal;  " target="_top"><cfif fileexists(request.zos.globals.homedir&'images/content/'&qContent.content_thumbnail)><img src="#request.zos.globals.domain&'/images/content/'##qContent.content_thumbnail#" class="listing-d-img" id="zclistingdimg#qContent.content_id#" width="100" height="78"><cfelse>Image N/A</cfif><br /> #left(qContent.content_name,30)#<cfif len(qContent.content_name) GT 30>...</cfif></a><br /><a href="/z/listing/sl/index?saveAct=delete&amp;content_id=#qContent.content_id#<cfif structkeyexists(form, 'returnURL')>&amp;returnURL=#URLEncodedFormat(form.returnURL)#</cfif>" rel="nofollow" style=" font-weight:bold;  " title="Delete This Property From Your List" target="_top">Remove</a></td>
            </cfloop>
            </tr>
	</table>
</cfif>
...
<cfif form.saveAct eq 'check'><cfscript>application.zcore.tracking.backOneHit();
	if(structkeyexists(form, 'listing_id')){
		...
		if(structkeyexists(session.zos.listing.savedListingStruct, form.listing_id) EQ false){
			session.zos.listing.savedListingStruct[form.listing_id]=true;
			application.zcore.listingCom.updateUserSavedListings();
		}
		ts=structnew();
		ts.name="savedListingStruct";
		ts.value=structkeylist(session.zos.listing.savedListingStruct);
		ts.expires="never";
		application.zcore.functions.zCookie(ts);
		ts=structnew();
		ts.name="savedListingCount";
		ts.value=structcount(session.zos.listing.savedListingStruct);
		ts.expires="never";
		application.zcore.functions.zCookie(ts);
		//cookie.savedListingCount=structcount(session.zos.listing.savedListingStruct);
		...
		if(structkeyexists(session.zos.listing.savedcontentStruct, form.content_id) EQ false){
			session.zos.listing.savedcontentStruct[form.content_id]=true;
			application.zcore.listingCom.updateUserSavedListings();
		}
		ts=structnew();
		ts.name="savedContentStruct";
		ts.value=structkeylist(session.zos.listing.savedContentStruct);
		ts.expires="never";
		application.zcore.functions.zCookie(ts);
		ts=structnew();
		ts.name="savedcontentCount";
		ts.value=structcount(session.zos.listing.savedcontentStruct);
		ts.expires="never";
		application.zcore.functions.zCookie(ts);
		//cookie.savedcontentCount=structcount(session.zos.listing.savedcontentStruct);
		if(structkeyexists(form, 'returnURL')){
			application.zcore.functions.zRedirect(form.returnURL);
		}else{
			application.zcore.functions.zRedirect(application.zcore.functions.zURLAppend(request.zos.listing.functions.getSearchFormLink(), 'searchid='&application.zcore.functions.zso(form, 'searchId')));
...
<cfif form.saveAct eq 'inquiry'>
	<cfscript>
	form.listing_id=structkeylist(session.zos.listing.savedListingStruct);
	form.content_id=structkeylist(session.zos.listing.savedContentStruct);
...
<cfif form.saveAct eq 'delete'><cfscript>application.zcore.tracking.backOneHit();
	if(structkeyexists(form, 'listing_id')){
		structdelete(session.zos.listing.savedListingStruct, form.listing_id);
		application.zcore.listingCom.updateUserSavedListings();
		ts=structnew();
		ts.name="savedListingCount";
		ts.value=structcount(session.zos.listing.savedListingStruct);
		...
		structdelete(session.zos.listing.savedcontentStruct, form.content_id);
		application.zcore.listingCom.updateUserSavedListings();
		ts=structnew();
		ts.name="savedcontentCount";
		ts.value=structcount(session.zos.listing.savedcontentStruct);
