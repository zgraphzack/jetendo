...
DELETE FROM `#request.zos.zcoreDatasource#`.rets17_property where rets17_mls_acct LIKE '17-%';
		...
		*/
		...
		if(application.zcore.functions.zso(ts, 'rets17_latitude',true) NEQ ''){
			...
			if(trim(address) NEQ ""){// and cityName NEQ ""){
				rs5=this.baseGetLatLong(address,application.zcore.functions.zso(ts, 'rets17_state'),application.zcore.functions.zso(ts, 'rets17_zip'));
				curLat=rs5.latitude;
    ...
    <cffunction name="getJoinSQL" localmode="modern" output="yes" returntype="any">
    	...
    	<cfreturn "INNER JOIN #db.table("rets17_property", request.zos.zcoreDatasource)# rets17_property ON rets17_property.rets17_mls_acct = listing.listing_id">
    </cffunction>
