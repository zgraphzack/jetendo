...
DELETE FROM `#request.zos.zcoreDatasource#`.rets7_property where rets7_175 LIKE '7-%';
		...
		*/
		...
		curLong='';
		if(trim(address) NEQ ""){
			rs5=this.baseGetLatLong(address,ts['state id'],ts['zip code'], arguments.ss.listing_id);
			curLat=rs5.latitude;
    ...
    <cffunction name="getJoinSQL" localmode="modern" output="yes" returntype="any">
    	...
    	<cfreturn "#arguments.joinType# JOIN #db.table("rets7_property", request.zos.zcoreDatasource)# rets7_property ON rets7_property.rets7_175 = listing.listing_id">
    </cffunction>
