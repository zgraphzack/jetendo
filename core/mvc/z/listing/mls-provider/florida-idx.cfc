<cfcomponent extends="base">
    ...
    <cffunction name="getLookupTables" localmode="modern" access="public" output="no" returntype="struct">
		...
		for(i in ts){
			arrayappend(arrSQL,"('9','listing_type','#ts[i]#','#i#','#request.zos.mysqlnow#','#i#')");
		}
		...
		for(i in ts){
			arrayappend(arrSQL,"('9','style','#ts[i]#','#i#','#request.zos.mysqlnow#','#i#')");
		}
		...
		for(i in ts){
			arrayappend(arrSQL,"('9','view','#ts[i]#','#i#','#request.zos.mysqlnow#','#i#')");
		}
		...
		for(i in ts){
			arrayappend(arrSQL,"('9','frontage','#ts[i]#','#i#','#request.zos.mysqlnow#','#i#')");
		}
		...
		for(i=1;i LTE arraylen(arrSubType);i++){
			arrayappend(arrSQL,"('9','listing_sub_type','#arrSubType[i]#','#arrSubType[i]#','#request.zos.mysqlnow#','#arrSubType[i]#')");
		}
		...
		for(i=1;i LTE arraylen(arrCounty);i++){
			arrayappend(arrSQL,"('9','county','#arrCounty[i]#','#arrCounty[i]#','#request.zos.mysqlnow#','#arrCounty[i]#')");
		}
